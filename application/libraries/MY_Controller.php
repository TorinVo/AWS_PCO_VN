...
class Controller extends Controller_Core
{
	...
	public function init_client(){		
		...
		$this->options_cus = array();
		if(!empty($this->sess_cus)){
			$this->db->where('member_id', $this->sess_cus['id']);
			$this->options_cus = $this->db->get('options')->result_array(false);
			$this->options_cus = !empty($this->options_cus)?$this->options_cus[0]:array();
		}
...
// End FOR Calendar
	...
	public function My_Update_Events($UpdateScheduling = array()){
		$sql_Select = 'SELECT MAX(Events.start_date) AS Events_StartDate, 
			Events.service_id AS ServiceID, Events.customer_id AS CustomerID, 
			Events.scheduling_id AS SchedulingID, Events.technician AS Events_Technician, 
			Events.start_time AS Events_StartTime, Events.start_date_auto_old AS Events_StartDate_Old, 
			Events.hours AS Events_Hours, Events.minutes AS Events_Minutes, 
			Scheduling.number_week_month AS NumberWeek, Scheduling.name_week_month AS NameWeekEvents, 
			Scheduling.frequency AS Scheduling_Frequency, Scheduling.end_condition AS Scheduling_EndCondition, 
			Scheduling.year_select AS Scheduling_YearSelect, 
			Scheduling.frequency_slt_week_1 AS Scheduling_Frequency_Week_1, 
			Scheduling.frequency_slt_week_2 AS Scheduling_Frequency_Week_2, 
			Scheduling.frequency_slt_nth_1 AS Scheduling_Frequency_Nth_1, 
			Scheduling.frequency_slt_nth_2 AS Scheduling_Frequency_Nth_2, 
			Scheduling.number_of_appointments AS Scheduling_NumberofApp, 
			Scheduling.date_mount_of_time AS Scheduling_DateTime, 
			Scheduling.value_mount_of_time AS Scheduling_ValTime, 
			Scheduling.option_scheduling AS Scheduling_OptionsScheduling, 
			Scheduling.chk_auto_schedule_work AS Scheduling_ChkAutoWork, 
			Scheduling.technician AS Scheduling_Technician, 
			Scheduling.start_time_time AS Scheduling_StartTime, 
			Scheduling.hours AS Scheduling_Hours, Scheduling.minutes AS Scheduling_Minutes, 
			Scheduling.show AS SchedulingShow, Scheduling.member_id AS SchedulingMemberId, 
			Scheduling.number_of_edits AS SchedulingNumberEdit, 
